%% init.m
% This code defines the problem setup for the subsequent inverse scattering
% routines to be tested. A specified computational domain (for now - just the unit cube), object under
% study, and impulse are specified here in order to carry out the forward
% scattering problem. Data from the forward problem is used to verify the
% accurarcy of the inverse scattering problem routine.
...
dim = length(x);               %single linear dimension; entire domain is (dim) x (dim) x (dim)
...
U_i = setup.createSources(XX,YY,ZZ,k,dim,Ntheta,Nphi);
...
[xd,yd,zd] = setup.createDetectors(Ndetect_x,Ndetect_y);
U_d = setup.detectorFields(XX,YY,ZZ,xd,yd,zd,dim,k,Ndetect_x,Ndetect_y);

%% Compute Green's Function
%compute the Green's function relating the detector to the scatterer for
%each point in the sample, for each detector.
G = setup.greensMatrix(XX,YY,ZZ,dim,k,h);
%G = (h^3).*G;
...
save('data/forwardProblem.mat','FWD');